...
模拟器不用于验证原生深度效果。本交付没有经过 Xcode SDK 构建或真机运行：本环境为 Linux，具体验证边界见下文。

## 这次实现的交互

进入后是普通相机画面，不运行实时虚化。顶部新增“拍后景深”开关和虚拟光圈，默认 f/1.8，可选 f/1.4～f/16；调节它只影响下一张照片。点按画面仍调用原有硬件对焦。

按快门后冻结本张照片的景深开关、虚拟光圈、方向和镜像信息，在同一次拍摄中获取照片与原生深度。等照片完成处理后，后台执行 Core Image 景深渲染，再显示成片。

结果页完整显示照片，标注“景深成片”或具体的普通照片降级原因，以及实际像素尺寸。景深成功时可按住“看原图”，这只是当前结果的内存对比，不是编辑器。点击保存写入完整分辨率的成片 JPEG；点击重拍清空本次内存数据并回到普通取景。

## 明确不做的事情

**不保存聚焦位置，不创建 Recipe，不保存可重编辑原图/深度档案，不实现拍后重选焦点，也不保证系统“照片”App 能做重新对焦。** 当前只把平面化成片保存到相册。快门时的临时原文件可能包含系统深度附件，但只在内存中用于本次处理；最终重新编码的 JPEG 不复制这些辅助附件和相机私有元数据。

硬件点按对焦保留，但本版没有指定自定义计算焦点矩形；景深滤镜采用系统默认/自动选择。**硬件对焦位置不等于本 Demo 已实现可控的算法焦平面。** 对杯子、叶片等非人脸主体，系统默认选择是否符合预期必须真机实测，不能把接口接入当作已复刻参考照片。

没有原生深度能力时仍可普通拍照，不用 AI 估计深度，也不采用全图高斯模糊或抠图后统一模糊背景来冒充景深。设备支持但本次没给深度、深度过于无效/没有明显层次、原生滤镜不可用或渲染失败时，也明确回退普通成片，不显示“景深成功”。
...
| `TestCamer/DepthPhotoProcessor.swift` | 新增拍后原生景深处理、深度有效性检查、对齐方向、生成显示预览及完整成片 JPEG。 |
...
本地执行环境为 Linux + Swift 6.2.1。已执行：6 个 App Swift 文件语法解析、25 条纯 Swift 值规则检查、9 项源码契约检查、Info.plist/pbxproj 语法检查和 XML/资源 JSON 解析。对应输出保存在 `Documentation/verification-local.txt`。

**Swift 语法解析不等于 iOS SDK 类型检查，源码契约检查也不等于运行时效果测试。尚未执行 Xcode 编译、签名安装、相机权限弹框操作、真实深度照片拍摄或视觉质量评估。** 因此不能声称这份工程已经在 iPhone 17 上运行通过，也不能声称效果已经与参考图一致。
...
后者构建通用 iOS 目标，不安装、不签名、不替代真机测试。手动操作项目运行更直接。真机测试步骤见 `Documentation/真机验收.md`。
